...
public class BayesNetPlayer_mfortman extends NannonPlayer {
	...
	@Override
	public String getPlayerName() { return "mfortman Bayes Net Player"; } // <------------------ choose a name for your player here in your (ranamed) copy of this class (ok to simply use your normal name or your initials, but also consider including your login name so unique).

	// Constructors.
	public BayesNetPlayer_mfortman() {
		initialize();

	}
	public BayesNetPlayer_mfortman(NannonGameBoard gameBoard) {
		super(gameBoard);
		initialize();
	}
	...
	int oppPrimeHasPrimeTotal = 0; 
	int oppHomeSafeNumTotal = 0;
	int oppSafeHomeNumTotal = 0;

	int primeSafeHomeWinTotal = 0;
	int primeSafeHomeLossTotal = 0;
	
	private int[]		oppPrime, 
						oppHome, 
						oppSafe; 
	
	private int[][]		oppPrimeHasPrime, 
						oppHomeSafeNum,
						oppSafeHomeNum;
	
	private int[][][] 	primeSafeHomeWin,
						primeSafeHomeLoss;

	private void initialize() {
		// Put things here needed for instance creation.
		oppPrime = new int[2];
		oppPrimeHasPrime = new int[2][2]; //hasPrime given oppPrime
		
		oppHome = new int[gameBoard.getPiecesPerPlayer() + 1];
		oppHomeSafeNum = new int[gameBoard.getPiecesPerPlayer() + 1][gameBoard.getPiecesPerPlayer() + 1]; //safeNum given oppHome
		
		oppSafe = new int[gameBoard.getPiecesPerPlayer() + 1];
		oppSafeHomeNum = new int[gameBoard.getPiecesPerPlayer() + 1][gameBoard.getPiecesPerPlayer() + 1]; //homeNum given oppSafe
		
		//win or loss given hasPrime, safeNum, and homeNum
		primeSafeHomeWin = new int[2][gameBoard.getPiecesPerPlayer() + 1][gameBoard.getPiecesPerPlayer() + 1];
		primeSafeHomeLoss = new int[2][gameBoard.getPiecesPerPlayer() + 1][gameBoard.getPiecesPerPlayer() + 1];
	}
	...
	public List<Integer> chooseMove(int[] boardConfiguration, List<List<Integer>> legalMoves) {
		...
		if (legalMoves != null) {		
			...
			for (List<Integer> move : legalMoves) { // <----- be sure to drop the "false &&" !
				...
				wins +=	primeSafeHomeWin[hasPrime][safeNum][homeNum];
				losses += primeSafeHomeLoss[hasPrime][safeNum][homeNum];
				//System.out.println(prob);
				
				prob = 	(((double) oppPrime[oppHasPrime] / (double) (wins + losses)) * 
						((double) oppPrimeHasPrime[oppHasPrime][hasPrime] / (double) (wins + losses)) *
						((double) oppHome[oppHomeNum] / (double) (wins + losses)) * 
						((double) oppHomeSafeNum[oppHomeNum][safeNum] / (double) (wins + losses)) * 
						((double) oppSafe[oppSafeNum] / (double) (wins + losses)) *
						((double) oppSafeHomeNum[oppSafeNum][homeNum] / (double) (wins + losses)) *
						((double) primeSafeHomeWin[hasPrime][safeNum][homeNum] / (double) (wins + losses)) *
						((double) wins / (double) (wins + losses))) /
						(((double) oppPrime[oppHasPrime] / (double) (wins + losses)) * 
						((double) oppPrimeHasPrime[oppHasPrime][hasPrime] / (double) (wins + losses)) *
						((double) oppHome[oppHomeNum] / (double) (wins + losses)) * 
						((double) oppHomeSafeNum[oppHomeNum][safeNum] / (double) (wins + losses)) * 
						((double) oppSafe[oppSafeNum] / (double) (wins + losses)) *
						((double) oppSafeHomeNum[oppSafeNum][homeNum] / (double) (wins + losses)) *
						((double) primeSafeHomeLoss[hasPrime][safeNum][homeNum] / (double) (wins + losses)) *
	...
	public void updateStatistics(boolean             didIwinThisGame, 
		...
		for (int myMove = 0; myMove < numberOfMyMovesThisGame; myMove++) {
			...
			oppPrime[oppHasPrime]++;
			oppPrimeHasPrime[oppHasPrime][hasPrime]++;
			
			oppHome[oppHomeNum]++;
			oppHomeSafeNum[oppHomeNum][safeNum]++;
			
			oppSafe[oppSafeNum]++;
			oppSafeHomeNum[oppSafeNum][homeNum]++;

			//update feature arrays according to win vs loss
			if (didIwinThisGame) {
				primeSafeHomeWin[hasPrime][safeNum][homeNum]++;
			} else {
				primeSafeHomeLoss[hasPrime][safeNum][homeNum]++;
			}
		}
