...
public abstract class NannonPlayer {
	...
	// Players might want to use this to convert the number for a cell to handle HOME (H) and SAFE (S), assuming the board cells are converted to 
	//zero-based counting and the extra array location is used to represent HOME or SAFE (moves cannot be from SAFE nor be to HOME, so no need to 
	//worry about 'collisions' by over using one memory cell).
