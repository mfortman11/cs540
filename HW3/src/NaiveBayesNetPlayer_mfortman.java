...
public class NaiveBayesNetPlayer_mfortman extends NannonPlayer {
	...
	@Override
	public String getPlayerName() { return "mfortman Naive Bayes Net Player"; } // <------------------ choose a name for your player here in your (ranamed) copy of this class (ok to simply use your normal name or your initials, but also consider including your login name so unique).

	// Constructors.
	public NaiveBayesNetPlayer_mfortman() {
		initialize();

	}
	public NaiveBayesNetPlayer_mfortman(NannonGameBoard gameBoard) {
		super(gameBoard);
		initialize();
	}

	private int[] 	homeNumWin, homeNumLoss,
					safeNumWin, safeNumLoss,
					hasPrimeWin, hasPrimeLoss,
					oppHomeNumWin, oppHomeNumLoss,
					oppSafeNumWin, oppSafeNumLoss,
					oppHasPrimeWin, oppHasPrimeLoss;

	private void initialize() {
		// Put things here needed for instance creation.
		homeNumWin = new int[gameBoard.getPiecesPerPlayer() + 1];
		homeNumLoss = new int[gameBoard.getPiecesPerPlayer() + 1];

		safeNumWin = new int[gameBoard.getPiecesPerPlayer() + 1];
		safeNumLoss = new int[gameBoard.getPiecesPerPlayer() + 1];

		hasPrimeWin = new int[2];
		hasPrimeLoss = new int[2];

		oppHomeNumWin = new int[gameBoard.getPiecesPerPlayer() + 1];
		oppHomeNumLoss = new int[gameBoard.getPiecesPerPlayer() + 1];

		oppSafeNumWin = new int[gameBoard.getPiecesPerPlayer() + 1];
		oppSafeNumLoss = new int[gameBoard.getPiecesPerPlayer() + 1];

		oppHasPrimeWin = new int[2];
		oppHasPrimeLoss = new int[2];
	}
			...
			for (List<Integer> move : legalMoves) { // <----- be sure to drop the "false &&" !
				...
				if(resultingBoard[0] == 1) {
					...
					if (extendsPrimeOfMine || createsPrimeOfMine)
						...
						hasPrime = 0;
						for (int i = 0; i < gameBoard.getCellsOnBoard()-1; i++) {
				...
				wins += homeNumWin[homeNum];
				wins += safeNumWin[safeNum];
				wins += hasPrimeWin[hasPrime];
				wins += oppHomeNumWin[oppHomeNum];
				wins += oppSafeNumWin[oppSafeNum];
				wins += oppHasPrimeWin[oppHasPrime];

				losses += homeNumLoss[homeNum];
				losses += safeNumLoss[safeNum];
				losses += hasPrimeLoss[hasPrime];
				losses += oppHomeNumLoss[oppHomeNum];
				losses += oppSafeNumLoss[oppSafeNum];
				losses += oppHasPrimeLoss[oppHasPrime];
				
				prob = 	(((double) homeNumWin[homeNum] / (double) (wins)) * 
						((double) safeNumWin[safeNum] / (double) (wins)) *
						((double) hasPrimeWin[hasPrime] / (double) (wins)) * 
						((double) oppHomeNumWin[oppHomeNum] / (double) (wins)) * 
						((double) oppSafeNumWin[oppSafeNum] / (double) (wins)) *
						((double) oppHasPrimeWin[oppHasPrime] / (double) (wins)) *
						((double) wins / (double) (wins + losses))) / 
						(((double) homeNumLoss[homeNum] / (double) (losses)) * 
						((double) safeNumLoss[safeNum] / (double) (losses)) *
						((double) hasPrimeLoss[hasPrime] / (double) (losses)) * 
						((double) oppHomeNumLoss[oppHomeNum] / (double) (losses)) * 
						((double) oppSafeNumLoss[oppSafeNum] / (double) (losses)) *
						((double) oppHasPrimeLoss[oppHasPrime] / (double) (losses)) *
						((double) losses / (double) (wins + losses)));
	...
	public void updateStatistics(boolean             didIwinThisGame, 
		...
		for (int myMove = 0; myMove < numberOfMyMovesThisGame; myMove++) {
			...
			//update feature arrays according to win vs loss
			if (didIwinThisGame) {
				homeNumWin[homeNum]++;
				safeNumWin[safeNum]++;
				hasPrimeWin[hasPrime]++;
				oppHomeNumWin[oppHomeNum]++;
				oppSafeNumWin[oppSafeNum]++;
				oppHasPrimeWin[oppHasPrime]++;
			} else {
				homeNumLoss[homeNum]++;
				safeNumLoss[safeNum]++;
				hasPrimeLoss[hasPrime]++;
				oppHomeNumLoss[oppHomeNum]++;
				oppSafeNumLoss[oppSafeNum]++;
				oppHasPrimeLoss[oppHasPrime]++;
			}
		}
